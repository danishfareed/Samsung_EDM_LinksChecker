    ...
    if (request.market) {
      ...
      var promises = links.map(function(link) {
        return new Promise(function(resolve) {
          var url = link.getAttribute('href').replace(/(https?:\/\/)?(www\.)?/, '$1');
          var isRelevant = relevantURLs[request.market].some(function(relevantURL) {
            return url.startsWith(relevantURL);
          });
  
          if (!isRelevant) {
            nonRelevantLinks.push({ href: url, element: link });
          }
      ...
      Promise.all(promises).then(function() {
        sendResponse({ nonRelevantLinks: nonRelevantLinks });
      });
