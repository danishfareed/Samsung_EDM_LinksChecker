...
function showAttributesTooltip(event) {
  ...
  // Get the target element (either <a> or <img>)
  const actualTarget = target.tagName.toLowerCase() === 'a' ? target : target.closest('a');

  // Check if the target element is allowed
  // if (!actualTarget || !allowedAttributes.some(attr => actualTarget.hasAttribute(attr))) return;

  const attributes = Array.from(actualTarget.attributes).filter(attribute => allowedAttributes.includes(attribute.name));
  ...
  tooltip.style.backgroundColor = '#f2f2f7';
  ...
  for (const attribute of attributes) {
    ...
    switch (attribute.name) {
      case 'data-sap-hpa-ceimo-ioi-link':
        labelText = 'INT Tag';
        break;
      case 'href':
        labelText = 'Link';
        break;
      case 'data-sap-hpa-ceimo-link-alias':
        labelText = 'Type';
        break;
      case '_label':
        labelText = 'Label';
    ...
    attributeElement.innerHTML = `<span style="color:#0052D4;font-weight: 700;margin-bottom: 10px;">${labelText}:</span> ${attribute.value}`;
...
// Include event listeners for both <a> and <img> tags
document.addEventListener('mouseover', showAttributesTooltip);
document.addEventListener('mouseout', hideAttributesTooltip);


// Include event listeners for <img> tags
document.addEventListener('mouseover', showAttributesTooltip);
document.addEventListener('mouseout', hideAttributesTooltip);
